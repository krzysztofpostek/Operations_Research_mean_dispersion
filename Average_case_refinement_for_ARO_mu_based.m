...
cvx_begin
    ...
    subject to
        sum(Demand_probabilities_mu.*(c'*(reshape(sum(repmat(Ordering_LDR_mu_d_shape,[1 2^T]).*repmat(Demands_scenarios_mu,[1 1 T]),1),[2^T T])') + sum((repmat(Coefficients_holding,[1 2^T]).*max(x_1+cumsum((reshape(sum(repmat(Ordering_LDR_mu_d_shape,[1 2^T]).*repmat(Demands_scenarios_mu,[1 1 T]),1),[2^T T])') - Demands_scenarios_mu(2:T+1,:),1),0) + repmat(Coefficients_backlogging,[1 2^T]).*max((-x_1-cumsum((reshape(sum(repmat(Ordering_LDR_mu_d_shape,[1 2^T]).*repmat(Demands_scenarios_mu,[1 1 T]),1),[2^T T])') - Demands_scenarios_mu(2:T+1,:),1)),0)),1))) <= objective_function_average;
