% Adjustable robust optimization - linear decision rules
cvx_begin
    ...
    subject to
        sum(Demand_probabilities_mu_d.*(c'*(reshape(sum(repmat(Ordering_LDR_mu_d_shape,[1 3^T]).*repmat(Demands_scenarios_mu_d,[1 1 T]),1),[3^T T])') + sum((repmat(Coefficients_holding,[1 3^T]).*max(x_1+cumsum((reshape(sum(repmat(Ordering_LDR_mu_d_shape,[1 3^T]).*repmat(Demands_scenarios_mu_d,[1 1 T]),1),[3^T T])') - Demands_scenarios_mu_d(2:T+1,:),1),0) + repmat(Coefficients_backlogging,[1 3^T]).*max((-x_1-cumsum((reshape(sum(repmat(Ordering_LDR_mu_d_shape,[1 3^T]).*repmat(Demands_scenarios_mu_d,[1 1 T]),1),[3^T T])') - Demands_scenarios_mu_d(2:T+1,:),1)),0)),1))) <= objective_limit + 0.1;

        for t=1:T
            Ordering_LDR(t,2:T+1) == zeros(1,T);
        end
